...
#ifdef _CUDA
   attributes(managed) :: A0_2
   attributes(managed) :: A0_3
...
!$cuf kernel do(3) <<<*,*>>>
#else
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i, j, k, l, p, q, r )      &
!$OMP&                          SHARED(feq, rho, u, v, w, weights, H2, ibgk, H3, vel, A0_2, A0_3)
#endif
   do k=1,nz
      do j=1,ny
         do i=1,nx
...
! Equilibrium distribution \citet{fen21a} Eq. (32) or jac18a eq (27)
            ...
            do q=1,3
               do l=1,nl
                  feq(l,i,j,k)=feq(l,i,j,k) + H2(p,q,l)*A0_2(p,q,i,j,k)/(2.0*cs4)
               enddo
            enddo
            enddo
!            call dgemv('t', 9,nl,inv2cs4,H2,9,A0_2,1,1.0,feq(1,i,j,k),1)  !! CHECK
...
! the above identically recovers the BGK equilibrium, now we add third order contributions
            if (ibgk == 3) then
               do l=1,nl
                     ...
                     enddo
               enddo
!               call dgemv('t',27,27,inv6cs6,H3,27,A0_3,1,1.0,feq(1,i,j,k),1)
            endif
