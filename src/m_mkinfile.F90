module m_mkinfile
contains
subroutine mkinfile()

open(10,file='infile.in',status='new')
   write(10,'(a)')'# Development variables'
   write(10,'(a)')' T                ! ltiming         : CPU timing'
   write(10,'(a)')' F                ! ltesting        : testing final solution while developing code.'
   write(10,'(a)')' T                ! ldump           : Saves diagnostics and restarts files'
   write(10,'(a)')' 256 1 1          ! ntx, nty, ntz   : number of threads per block in x, y, and z direction'
   write(10,'(a)')' F                ! lmeasurements   : Saves predicted measurements for data assimilation experiments'
   write(10,'(a)')'# Experiment configuration'
   write(10,'(a)')' city2            ! experiment      : Cylinder, airfoil etc'
   write(10,'(a)')' 3                ! ibgk            : BGK order, ibgk=2 standard second order, ibgk=3 gives third order BGK'
   write(10,'(a)')' 1                ! ihrr            : Collision operator ihrr=1 includes regularization by 3rd ord Hermite'
   write(10,'(a)')' 1 0.15           ! ivreman smagor  : Vreman subgridscale mixing (1) with Smagorinsky constant (0.15)'
   write(10,'(a)')'# Atmospheric boundary layer'
   write(10,'(a)')' 0                ! iablvisc        : mode for ABL (0-none, 1-mechanical layer, 2-buoyancy scheme)'
   write(10,'(a)')' 400.0            ! ablheight       : height of ABL in meters'
   write(10,'(a)')' 1                ! istable         : Stability of ABL when iablvisc=2 (1=stable, 0=netural, -1=unstable)'
   write(10,'(a)')'# Time stepping and outputs'
   write(10,'(a)')' 00000            ! nt0             : First timestep'
   write(10,'(a)')' 1000             ! nt1             : Final timestep'
   write(10,'(a)')' 5000             ! iout            : Number of steps between diag outputs'
   write(10,'(a)')' 10000            ! irestarts       : Number of steps between restart outputs'
   write(10,'(a)')' 00 60000 1       ! iprt1 iprt2 x   : Output every x timestep for it <= iprt1 and it >= iprt2'
   write(10,'(a)')' 2                ! tecout          : full tecplot files (0), only solution (2), netcdf (3)'
   write(10,'(a)')'# Boundary conditions                                            Closed for ijk=1 and ijk=n(xyz) boundaries'
   write(10,'(a)')' 1                ! ibnd            : 0-periodic, 1-in/out flow, 11,12,21,22 no-slip (1), free-slip (2)'
   write(10,'(a)')' 0                ! jbnd            : 0-periodic, 1-in/out flow, 11,12,21,22 no-slip (1), free-slip (2)'
   write(10,'(a)')' 22               ! kbnd            : 0-periodic,                11,12,21,22 no-slip (1), free-slip (2)'
   write(10,'(a)')'# Inflow variables'
   write(10,'(a)')' 8.0 0.0          ! uini, udir      : Inflow wind velocity [m/s], direction in degrees (-45:45)'
   write(10,'(a)')' F 0.00005  100   ! lturb amp nrtu  : Add turbulence forcing on inflow, amplitude, number of prestored time ste'
   write(10,'(a)')'# Physical variables'
   write(10,'(a)')' 0.0000178        ! visckin         : Dimensional kinematic viscosity'
   write(10,'(a)')' 1.225            ! C_rho - Density of air at surface 15C and  101.325 kPa  [kg/m^3] Eq. (7.12)'
   write(10,'(a)')' 4.0              ! C_l   - Length of a lattice cell in meters [m]'
   write(10,'(a)')' 75.0             ! C_u   - Wind velocity conversion [m/s]   -> C_t=C_l/C_u'
   write(10,'(a)')'# Averaging variables'
   write(10,'(a)')' F                ! lave            : Switch on/off averaging'
   write(10,'(a)')' 20000            ! avestart        : Iteration to start computing section averages'
   write(10,'(a)')' 40000            ! avestop         : Iteration to save section averages'
   write(10,'(a)')'# Turbine-definitions'
   write(10,'(a)')' 0                ! nturbines       : Number of turbines'
   write(10,'(a)')' NREL5MW          ! turbname        : Turbine type    '
   write(10,'(a)')' 0                ! actuator_model  : Actuator line (0), actuator disk (1)'
   write(10,'(a)')' 36               ! nazim           : Number of azimutal angles for computing ADM points'
   write(10,'(a)')' 1                ! local wind      : 1- activate yaw controller, 0-infile yaw values, 2-external wind direction'
   write(10,'(a)')' 1.0              ! dt_control      : yaw_controller: Seconds between each yaw-update'
   write(10,'(a)')' 0.3              ! yawrate_max     : yaw_controller: Maximum yaw update in degrees per second'
   write(10,'(a)')' 2.0              ! yaw_deadband    : yaw_controller: A threshold, in degrees, before yaw controller activates'
   write(10,'(a)')' 3.0              ! filter_time     : yaw_controller: time-filter of local wind average'
   write(10,'(a)')' 0.0              ! pitchangle      : Imposed pitchangle (0 until u=11.4, see table 7.1 in NREL doc).'
   write(10,'(a)')' 8.95             ! turbrpm         : Initial/fixed RPM; used when tipspeedratio <= 0'
   write(10,'(a)')' 7.55             ! tipspeedratio   : Target TSR; >0 overrides RPM using local upstream wind'
   write(10,'(a)')' 0.93             ! powerloss       : Powerloss factor for turbine drive chain and generator'
   write(10,'(a)')'# T1'
   write(10,'(a)')' 1 T1 250.0  480.0  90.0  0.0  0.0    ! n, name, xpos, ypos, hubheight, yaw, tilt'
   write(10,'(a)')' 2 T2 250.0  680.0  90.0  0.0  0.0    ! n, name, xpos, ypos, hubheight, yaw, tilt'
   write(10,'(a)')
   write(10,'(a)')
   write(10,'(a)')' kinematic viscosity of air is 1.78E-5'
   write(10,'(a)')' Reynolds number becomes Re= u D /nu = 10^1 * 10^2 / 10^(-5)  = 10^7 - 10^8'
close(10)

end subroutine
end module
